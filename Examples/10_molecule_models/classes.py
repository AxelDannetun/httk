...
class HostUnitCellResult(httk.Result):
    ...
    @httk.httk_typed_init({'host_name': str, 'computation': httk.Computation,  'structure': Structure, 'formation_entalpy': float, 'mp_id': str, 'dielectric_tensor_electric': [float], 'dielectric_tensor_ionic': [float], 'refractive_index': float}, index=['host_name'])
    def __init__(self, host_name, computation, structure, mp_id, formation_entalpy, dielectric_tensor_electric, dielectric_tensor_ionic, refractive_index):
